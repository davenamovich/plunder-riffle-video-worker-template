...
exports.getRecordingStatus = exports.listJobs = exports.getJob = exports.OUTPUT_DIMS = void 0;
exports.recordingsDir = recordingsDir;
exports.viewportDims = viewportDims;
exports.resolveChromium = resolveChromium;
exports.diagnoseRecorderEnvironment = diagnoseRecorderEnvironment;
exports.getRecorderHealth = getRecorderHealth;
exports.runFfmpeg = runFfmpeg;
exports.resolveAudioForMix = resolveAudioForMix;
exports.collectPageInfo = collectPageInfo;
exports.captureSlideshowStillFrames = captureSlideshowStillFrames;
exports.computeScreencastDurationMs = computeScreencastDurationMs;
exports.buildDriverConfig = buildDriverConfig;
exports.drivePageForRecording = drivePageForRecording;
exports.computeScreencastTrim = computeScreencastTrim;
exports.buildAudioMixFilter = buildAudioMixFilter;
exports.buildAppSlidesAudioMix = buildAppSlidesAudioMix;
exports.composeScreencastToMp4 = composeScreencastToMp4;
exports.jobTimeoutMs = jobTimeoutMs;
exports.acquireJobSlot = acquireJobSlot;
exports.releaseJobSlot = releaseJobSlot;
exports.getRenderQueueStatus = getRenderQueueStatus;
exports.setShuttingDown = setShuttingDown;
exports.isWorkerShuttingDown = isWorkerShuttingDown;
...
exports.OUTPUT_DIMS = { width: VERTICAL_OUT_W, height: VERTICAL_OUT_H };
const FPS = 30;
...
function resolveChromium() {
    ...
    if (configured) {
        if ((0, fs_1.existsSync)(configured))
            return configured;
        console.warn(`[recorder] PLAYWRIGHT_CHROMIUM_PATH points to a missing binary (${configured}) — ` +
 ...
 */
function playwrightBrowserPresent() {
    const candidates = [
        (0, path_1.join)(process.env.HOME || ".", ".cache", "ms-playwright"),
        (0, path_1.join)(process.env.LOCALAPPDATA || "", "ms-playwright"),
    ].filter((p) => p.length > 0);
    try {
 ...
 */
function diagnoseRecorderEnvironment() {
    ...
    if (ffmpegPath === "ffmpeg") {
        ...
        // loop while it runs).
        const probe = (0, child_process_1.spawnSync)("ffmpeg", ["-version"], { stdio: "ignore", timeout: 5000 });
        ffmpegFound = !probe.error && probe.status === 0;
    }
    else {
        ffmpegFound = (0, fs_1.existsSync)(ffmpegPath);
    }
    const configured = getEnv("PLAYWRIGHT_CHROMIUM_PATH");
    let chromiumFound;
    let chromiumSource;
    if (configured) {
        ...
        chromiumFound = (0, fs_1.existsSync)(configured);
    }
    else {
        ...
        ffmpeg.on("close", (code) => {
            ...
            if (process.env.DEBUG_FFMPEG) {
                try {
                    ...
                    const dump = (0, path_1.join)((0, os_1.tmpdir)(), `ffmpeg-debug-${Date.now()}.log`);
 ...
 */
function fillsOutputAspect(phoneW, phoneH, outW, outH) {
 ...
 */
function phoneRect(phoneW, phoneH, outW, outH) {
    const margin = 160;
 ...
 */
function phoneCropFilter(phoneW, phoneH, outW, outH) {
 ...
 */
function buildCompositeFilter(background, phoneW, phoneH, holdSec = 0, trimSec = 0, outW = VERTICAL_OUT_W, outH = VERTICAL_OUT_H, noFade = false) {
    const chains = [];
    ...
    if (fillsOutputAspect(phoneW, phoneH, outW, outH)) {
        ...
        chains.push(`[0:v]${trim}${hold}scale=${outW}:${outH}:force_original_aspect_ratio=increase,` +
    ...
    if (background === "blur") {
        ...
        chains.push(`[src_bg]scale=270:480:force_original_aspect_ratio=increase,crop=270:480,gblur=sigma=10,eq=brightness=-0.3,scale=${outW}:${outH}[bg]`);
...
async function collectPageInfo(page) {
    return page
        ...
        const musicSrc = document.getElementById("score")?.getAttribute("src") ||
...
async function captureAppSlidesFrames(browser, pageUrl, opts, beatCount, dir) {
    const vp = viewportDims((opts.viewport || "vertical"));
    ...
    for (let n = 0; n < beatCount; n++) {
        ...
        const p = (0, path_1.join)(dir, `appslide-${n}.jpg`);
 ...
 */
async function waitForBeatReady(page, maxMs) {
    await page
        .evaluate(async (maxWaitMs) => {
        ...
        if (typeof document.getAnimations === "function") {
            for (const anim of document.getAnimations()) {
                ...
                if (target && root.contains(target)) {
                    waits.push(anim.finished.catch(() => { }));
                }
            }
        }
 ...
 */
async function captureSlideshowStillFrames(pageUrl, opts = {}) {
    const { chromium, devices } = await Promise.resolve().then(() => __importStar(require("playwright-core")));
    ...
    try {
        ...
        await page.evaluate(() => document.fonts?.ready).catch(() => { });
        await page
            .waitForFunction(() => window.__vessel?.ready === true, { timeout: 10_000 })
            .catch(() => { });
        const pageInfo = await collectPageInfo(page);
        if (pageInfo.beatCount < 1)
            throw new Error("Slideshow capture: no beats found on page");
        ...
        for (let i = 0; i < pageInfo.beatCount; i++) {
            ...
            await page.evaluate((idx) => window.__vessel?.setBeat(idx), n).catch(() => { });
        ...
        await page
            .evaluate((idx) => window.__vessel?.setBeat(idx), ctaFrame)
            .catch(() => { });
...
async function assembleAppSlidesVideo(frames, durationsSec, outPath, outW, outH) {
    const n = frames.length;
    if (n < 1)
        throw new Error("assembleAppSlidesVideo: need >= 1 frame");
    ...
    const parts = [];
    for (let i = 0; i < n; i++) {
        parts.push(`[${i}:v]trim=duration=${durationsSec[i].toFixed(3)},setpts=PTS-STARTPTS,` +
    ...
    parts.push(`${finalLabel}tpad=stop_duration=${APPSLIDE_OUTRO_SEC.toFixed(3)}:stop_mode=clone[vfinal]`);
    finalLabel = "[vfinal]";
 ...
 */
function computeScreencastDurationMs(pageInfo, opts) {
 ...
 */
function buildDriverConfig(opts) {
    return {
 ...
 */
async function drivePageForRecording(page, opts) {
    // Auto-start playback
    await page
        .evaluate(() => {
 ...
 */
function computeScreencastTrim(input) {
    ...
    const targetDurSec = Math.min(EXPLICIT_DURATION_CAP_MS / 1000, Math.max(0.5, (input.durationMs + POST_ROLL_MS) / 1000));
 ...
 */
function buildAudioMixFilter(o) {
 ...
 */
function buildAppSlidesAudioMix(opts) {
    ...
    return {
        ...
        filter: `[1:a]aresample=44100,atrim=0:${videoDurSec.toFixed(3)},volume=${audioVol.toFixed(2)},` +
 ...
 */
async function composeScreencastToMp4(input) {
    const { webmPath, mp4Path, thumbPath, showStartMs, showEndMs, durationMs, viewport, background, audioPath, musicPath, outWidth = VERTICAL_OUT_W, outHeight = VERTICAL_OUT_H, audioVolume = 0.5, musicVolume, } = input;
    ...
    // regardless.
    const filter = buildCompositeFilter(background, viewport.width, viewport.height, holdSec, 0, outWidth, outHeight, input.isInteractive === true);
    ...
    const thumbCrop = outHeight / outWidth > 1
...
async function recordPage(opts, id) {
    ...
    const viewport = viewportDims((opts.viewport || "vertical"));
    ...
    try {
        ...
        const musicPath = await resolveAudioForMix(opts.musicUrl, pageInfo.musicSrc, opts.url, `${id}-music`);
        // APPSLIDES FAST PATH (faster-than-realtime). appslides is a deck of static
        ...
        if (pageInfo.appslidesVessel && pageInfo.beatCount >= 1) {
            ...
            if (frames.length < 1)
                throw new Error("AppSlides capture produced no frames");
            ...
            if (audioPath) {
                const mixedPath = (0, path_1.join)(workDir, "appslides-mixed.mp4");
                ...
                await (0, promises_1.rename)(mixedPath, mp4Path);
            }
            await assertMp4Healthy(mp4Path);
        ...
        const isInteractive = pageInfo.beatCount > 0 || pageInfo.vesselBeatCount > 0 || pageInfo.isInteractive;
        ...
        const composed = await composeScreencastToMp4({
            ...
            isInteractive: pageInfo.beatCount > 0 || pageInfo.vesselBeatCount > 0 || pageInfo.isInteractive,
            autoDuration,
            outWidth,
            outHeight,
            audioVolume: opts.audioVolume,
 ...
 */
function startRecordingJob(opts) {
 ...
 */
async function runJob(opts, runner) {
...
function sweepStaleJobs() {
    ...
    for (const [id, job] of jobs) {
        if (job.status === "recording" || job.status === "queued")
...
async function deleteOldRecordings(maxAgeMs, protectedIds) {
    ...
    try {
        const tDir = (0, os_1.tmpdir)();
        const tEntries = await (0, promises_1.readdir)(tDir, { withFileTypes: true });
        for (const entry of tEntries) {
            if (!entry.isDirectory() || !entry.name.startsWith('rec-'))
                continue;
            const dirPath = (0, path_1.join)(tDir, entry.name);
            try {
                const s = await (0, promises_1.stat)(dirPath);
                if (s.mtimeMs < cutoff) {
