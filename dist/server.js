...
function requireSecret(req, res, next) {
    if (!workerSecret)
        return next();
    if (req.get("x-operator-secret") === workerSecret)
        return next();
...
app.use("/download/recordings", express_1.default.static(path_1.default.join(process.cwd(), "download/recordings")));
...
app.post("/api/record", requireSecret, async (req, res) => {
    try {
        ...
        const job = await (0, recorder_1.startRecordingJob)(opts);
...
app.get("/api/record/:id", requireSecret, (req, res) => {
    const job = (0, recorder_1.getJob)(req.params.id);
    if (!job) {
...
app.get("/api/record/:id/download", (req, res) => {
    ...
    if (!fs_1.default.existsSync(job.result.mp4Path)) {
...
app.get("/api/record/:id/thumbnail", (req, res) => {
    ...
    if (!fs_1.default.existsSync(job.result.thumbnailPath)) {
...
app.delete("/api/record/:id", requireSecret, async (req, res) => {
    try {
        await (0, recorder_1.deleteJob)(req.params.id);
...
app.get("/health", async (_req, res) => {
    let recorder = null;
    try {
        recorder = await (0, recorder_1.recorderDiagnostics)();
    }
    catch (error) {
        console.warn("[worker] health diagnostic failed:", error.message);
    }
